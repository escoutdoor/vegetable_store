...
import (
	...
	"github.com/escoutdoor/vegetable_store/user_service/internal/metrics"
...
func (a *App) initGatewayServer(ctx context.Context) error {
	...
	if err := userv1.RegisterUserServiceHandlerFromEndpoint(ctx, gwMux, a.config.GRPC.Address(), opts); err != nil {
		return errwrap.Wrap("register user service handler from endpoint", err)
	}

	if err := authv1.RegisterAuthServiceHandlerFromEndpoint(ctx, gwMux, a.config.GRPC.Address(), opts); err != nil {
